...
#  Copyright 2020 Vertexcom Technologies, Inc.
#
...
logger = logging.getLogger('vc-platform')
...
class Element:
    def __init__(self, data, stream_kwargs, name=None):
        ...
        if patch_file:
            patch_file = os.path.join(PLATFORM_ROOT, patch_file)
        ...
        if dst:
            ...
            dst = os.path.join(PLATFORM_ROOT, dst)
    ...
    def apply_patch(self):
        if self.patch_file:
            ...
            if not is_integrated and self.source.__class__.__name__ == 'GitSource':
                check_cmd(['git', 'config', 'user.name', 'vc-platform'], cwd=self.destination, **self.stream_kwargs)
...
class Target(Element):
    """
    Class which describes supported target and it's operations.
    For supported targets run::

        vc-platform deploy -h

    Target's operations are

    * Writing deployment instructions to stdout/file.

    * Fetching the required target-dependent sources according to vc-platform.json.

    * Deleting fetched target-dependent sources according to vc-platform.json.

    * Apply patches if needed on the deployed sources according to vc-platform.json.

    :param name: Target name
    :param data: Dictionary describing the target (read from vc-platform.json)
    :param stream_kwargs:
        * *stdout* --
          Standard output handle
        * *stderr* --
    ...
    def fetch_elements(self):
        """
        Fetch the required target-dependent sources for the target according to vc-platform.json
        """
        self.fetch()
        self.os.fetch()
        self.device.fetch()
    ...
    def delete_elements(self):
        """
        Delete the required target-dependent sources of the target according to vc-platform.json
        """
        self.os.delete()
        self.device.delete()
        for mw in self.middleware:
    ...
    def patch_elements(self):
        """
        Apply patches if needed on the deployed sources of the target according to vc-platform.json
        """
        self.apply_patch()
        self.os.apply_patch()
...
@click.option(
    ...
    default=os.path.join(SCRIPT_DIR, 'vc-platform.json'),
    help='Path to a .json file containing the supported targets configuration.\n'
         'Default is %s' % os.path.normpath(os.path.join(SCRIPT_DIR, 'vc-platform.json'))
)
...
@cli.command(
    ...
    short_help='Deploy vcsoftware files (run "%s deploy -h" for help)' % PROG_NAME
)
@click.option(
    '--target',
    'target_name',
...
def deploy(config, target_name, skip_update, instructions):
    ...
    instructions_file = os.path.join(PLATFORM_ROOT, target.name + '.txt')
