...
This document describes how to write and use unit tests for MtOS. 
...
Find unit tests in the MtOs repository under the `tests/unit` folder. We recommend unit test files use an identical directory path as the file under test. This makes it easier to find unit tests for a particular class or a module. For example, if the file you're testing is `some/example/path/ClassName.cpp`, then all the test files are in the `tests/unit/some/example/path/ClassName` directory. Each test suite needs to have its own `unittest.cmake` file for test configuration.

All the class stubs should be located in the `tests/unit/stubs` directory. A single stub class can be used by multiple test suites and should follow the naming convention `ClassName_stub.cpp` for the source file, and `ClassName_stub.h` for the header file. Use the actual header files for the unit tests, and don't stub headers if possible. The stubbed headers reside in the `tests/unit/target_h` directory.

#### Test discovery

Registering unit tests to run happens automatically, and the test runner handles registration. However, test files do not automatically  build. Build unit tests with a separate system that searches for unit tests under the `tests/unit` directory.
...
The build system automatically generates names of test suites. The name is constructed by taking a relative file path from the tests/unit directory to the test directory and replacing path separators with dashes. For example, the test suite name for `some/example/path/ClassName.cpp` is `some-example-path-ClassName`. Suite names are used when deciding which test suites to run.
...
List all the required files for the build in the `unittest.cmake` file with paths relative to the `tests/unit` folder. Use the following variables to list the source files and include paths:

* **unittest-includes**: List of header include paths. You can use this to extend or overwrite default paths listed in `tests/unit/CMakeLists.txt`.
...
1. Create the following dummy classes in `mtos/example`:
...
1. Create a directory for MyClass unit tests in `tests/unit/example/MyClass`.
1. Create a configuration file and a source file for MyClass unit tests in `tests/unit/example/MyClass`:
...
1. Stub all external dependencies. Create the following stub in `tests/unit/stubs`:
...
This example does not use any MtOS code, but if your unit tests do, then remember to update header stubs in `tests/unit/target_h` and source stubs in `tests/unit/stubs` with any missing type or function declarations. 
...
1. Create a build directory `mkdir tests/unit/build`.
1. Move to the build directory `cd tests/unit/build`.
1. Run CMake using a relative path to `tests/unit` folder as the argument. So from `tests/unit/build` use `cmake ..`:
